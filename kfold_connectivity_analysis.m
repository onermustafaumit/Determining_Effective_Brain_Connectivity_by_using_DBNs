...
disleksi_before = xlsread('kfold_connectivity_distances.xls','dyslexia_before','A1:CV10');
disleksi_after = xlsread('kfold_connectivity_distances.xls','dyslexia_after','A1:CV10');
kontrol_before = xlsread('kfold_connectivity_distances.xls','control_before','A1:CV10');
kontrol_after = xlsread('kfold_connectivity_distances.xls','control_after','A1:CV10');

%%
threshold='09';
coeff=10;       %set threshold to choose a reachable edge as common if it exists in coeff=<# number of subjects
dist_upper_limit=3;     % set upper limit on the distance to connect two nodes, connection, whose two nodes connected with less than dist_upper_limit, is valid
std_corr_coeff=9/(coeff-1);
std_offset_corr_coeff=(10-coeff)/(coeff-1);

foldername=strcat('Kfold_Common_Reachable_Connections_by_',num2str(coeff),'_10th_of_subjects_threshold_',threshold);
...
%% kontrol before (kb)
num_of_common_conn_kb=sum(kontrol_before>0 & kontrol_before<2); %dist_upper_limit);
...
label=['Reachable Connections in ' num2str(coeff) '/10 of Kfold Control Before Models'];
...
title('\mu±\sigma for Kfold Control Before Distances')

%% kontrol after (ka)
num_of_common_conn_ka=sum(kontrol_after>0 & kontrol_after<2); %dist_upper_limit);
...
label=['Reachable Connections in ' num2str(coeff) '/10 of Kfold Control After Models'];
...
title('\mu±\sigma for Kfold Control After Distances')
...
label=['Reachable Connections in ' num2str(coeff) '/10 of Kfold Dyslexia Before Models'];
...
title('\mu±\sigma for Kfold Dyslexia Before Distances')
...
label=['Reachable Connections in ' num2str(coeff) '/10 of Kfold Dyslexia After Models'];
...
title('\mu±\sigma for Kfold Dyslexia After Distances')
